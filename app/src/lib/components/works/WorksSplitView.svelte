<script lang="ts">
	...
	let { works = [] }: { works: WorksData[] } = $props();
	...
	const listMaskImage = $derived(
		`linear-gradient(to bottom, ${canScrollUp ? 'transparent 0%, black 35%' : 'black 0%'}, ${canScrollDown ? 'black 65%, transparent 100%' : 'black 100%'})`
	);
	...
	const activeWork = $derived(works[activeIndex] ?? null);
	const previewStill = $derived(activeWork?.stills[0] ?? null);
...
<div class="relative flex h-full w-full flex-row bg-white">
	...
	<button
		...
		disabled={!activeWork}
		onclick={() => activeWork && goto(resolve(`/${activeWork.id}`))}
	>
		<div
